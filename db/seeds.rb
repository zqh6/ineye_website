...
def init_dictionary
  ActiveRecord::Base.transaction do
    ...
    file.each_line do |line|
      ...
      dictionary = Dictionary.included_by(array[0].strip).first
      dictionary = Dictionary.new if dictionary.blank?
      dictionary.assign_attributes id: array[0].strip, dictionary_type: array[1].strip, code: array[2].strip, name: array[3].strip, value: array[4].to_i
      dictionary.save
    end
  end
end
...
def create_user(phone_number: nil, password_str: nil)
    ...
    user.role_code  = RoleEnums.first.code
    user.save!
...
ActiveRecord::Base.transaction do
  init_dictionary
  create_user phone_number: '19999999999', password_str: '123456'
end
