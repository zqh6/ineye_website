class Administration::V1::LeavingsController < Administration::V1::PrivilegedController
  ...
  def new
    if params[:function]=='all_user'
      ...
      @ask_for_leaves_query = AskForLeave.alive.reorder('created_at DESC').paginate(page: params[:page], per_page: 10)
      @ask_for_leaves = show_ask_for_leaves @ask_for_leaves_query
      render action: 'new' and return
    elsif params[:function]=='one_user'
      @offices = @login_user.get_offices
      @ask_for_leaves_query = AskForLeave.alive.where(user_id: @login_user.id).reorder('created_at DESC').paginate(page: params[:page], per_page: 10)
      @ask_for_leaves = show_ask_for_leaves @ask_for_leaves_query
  ...
  def show_ask_for_leaves(ask_for_leaves)
    ...
    ask_for_leaves.each do |ask_for_leave|
      ...
      ask_for_leave_item[:office_name]=Office.where(id: ask_for_leave.office_id).first.try(:name)
