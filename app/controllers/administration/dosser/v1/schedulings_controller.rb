  ...
  def create
    ActiveRecord::Base.transaction do
      if params[:function].blank?
        ...
        collection = []
        params[:scheduling_info].each do |scheduling_info|
          office_time = OfficeTime.find(scheduling_info[:office_time_id])
          render_conflict message: 'office_time_id参数不正确' and return if office_time.blank?
          render_conflict message: 'outpatient_service_type参数不正确' and return if ShareEnum.outpatient_services[scheduling_info[:outpatient_service_type].to_sym].blank?
          render_conflict message: 'week参数不正确' and return if ShareEnum.weeks[scheduling_info[:week].to_sym].blank?

          scheduling = Scheduling.new office_time_id: scheduling_info[:office_time_id], user_id: @login_user.id, week_code: scheduling_info[:week], outpatient_service_type: scheduling_info[:outpatient_service_type]
          if scheduling.save
            collection.push({sheduling_id: scheduling.id})
          else
            render_conflict message: error_message(scheduling) and return
          end
        end
