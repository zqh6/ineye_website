class AllApi::UsersController < AllApi::PresentationController
  ...
  def create
    ActiveRecord::Base.transaction do
      ...
      render_conflict message: '请填写正确的手机号' and return if phone_number.blank?
      render_conflict message: '手机号格式不正确' and return if (/\A1\d{10}\z/.match(phone_number)).blank?
      ...
      render_conflict message: '真实姓名不能为空' and return if params[:name].blank?
      render_conflict message: '医院不能为空' and return if params[:unit_name].blank?
      #验证手机、手机验证码
      ...
      user = User.new role_code: 'outer_user', name: params[:name].strip, unit_name: params[:unit_name], phone_number: phone_number
      if user.save
